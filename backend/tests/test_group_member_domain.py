...
def create_user(client: TestClient, display_name: str = "owner") -> str:
    response = client.post("/api/v1/users", json={"display_name": display_name})
    assert response.status_code == 201
    return str(response.json()["user_id"])


def create_group(client: TestClient, user_id: str) -> dict[str, object]:
    response = client.post(
        "/api/v1/groups",
        headers={"X-UFC-User-Id": user_id},
...
def add_member(client: TestClient, user_id: str, group_id: str, name: str, mbti: str) -> dict[str, object]:
    response = client.post(
...
def test_user_group_and_members_can_be_created_and_group_becomes_ready() -> None:
    ...
    group = create_group(client, user_id)

    assert group["member_count"] == 0
    assert group["status"] == "DRAFT"
    assert group["can_analyze"] is False

    first_member = add_member(client, user_id, str(group["group_id"]), "민지", "ENFP")
    second_member = add_member(client, user_id, str(group["group_id"]), "준호", "ISTJ")

    assert first_member["mbti"] == "ENFP"
    assert second_member["display_name"] == "준호"

    response = client.get(
        f"/api/v1/groups/{group['group_id']}", headers={"X-UFC-User-Id": user_id}
    )
...
def test_group_member_count_must_not_exceed_four() -> None:
    ...
    for index, mbti in enumerate(["ENFP", "ISTJ", "INTP", "ESFJ"], start=1):
        add_member(client, user_id, str(group["group_id"]), f"member-{index}", mbti)
...
def test_group_returns_to_draft_when_member_count_drops_below_two() -> None:
    ...
    group = create_group(client, user_id)
    first_member = add_member(client, user_id, str(group["group_id"]), "민지", "ENFP")
    add_member(client, user_id, str(group["group_id"]), "준호", "ISTJ")
...
def test_other_users_cannot_access_group() -> None:
    ...
    other_user_id = create_user(client, "other")
    group = create_group(client, owner_id)

    response = client.get(
        f"/api/v1/groups/{group['group_id']}", headers={"X-UFC-User-Id": other_user_id}
    )
...
def test_unknown_user_cannot_list_groups() -> None:
    ...
    response = client.get("/api/v1/groups", headers={"X-UFC-User-Id": "missing-user"})
...
def test_member_can_be_updated_and_deleted() -> None:
    ...
    user_id = create_user(client)
    group = create_group(client, user_id)
    member = add_member(client, user_id, str(group["group_id"]), "민지", "ENFP")
...
def test_names_are_trimmed_and_blank_names_are_rejected() -> None:
    client = build_sqlite_client()
    user_id = create_user(client, " owner ")
...
def test_postgres_user_group_member_flow() -> None:
    ...
    user_id = create_user(client, "pg-owner")
    group = create_group(client, user_id)
    add_member(client, user_id, str(group["group_id"]), "pg-member-1", "ENFP")
    add_member(client, user_id, str(group["group_id"]), "pg-member-2", "ISTJ")
...
def test_postgres_concurrent_add_member_stops_at_four() -> None:
    client = build_postgres_client()
    user_id = create_user(client, "pg-concurrent-owner")
    group = create_group(client, user_id)
    for index, mbti in enumerate(["ENFP", "ISTJ", "INTP"], start=1):
        add_member(client, user_id, str(group["group_id"]), f"pg-existing-{index}", mbti)
