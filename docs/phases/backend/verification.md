...
- `pytest`: Passed, `33 passed, 1 warning`.
...
- BE Phase 3 security tests cover SEC-01 through SEC-07, login rate limiting, refresh, logout, and CORS origin restriction.
...
- Login rate limiting is in-memory and must move to shared storage before multi-process production deployment.
